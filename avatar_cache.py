"""Persistent, checksum-backed Avatar response library (Phase 3)."""
...
class AvatarCache:
    def __init__(self, root: str | Path = "data/avatar/library"):
        self.root = Path(root).resolve()
        self.media = self.root / "media"
        self.quarantine_dir = self.root / "quarantine"
        self.db_path = self.root / "avatar.sqlite3"
    ...
    def export_bundle(self, destination: str | Path) -> Path:
        ...
        with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
            archive.write(self.db_path, "avatar.sqlite3")
    ...
    def import_bundle(self, bundle: str | Path) -> int:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            source_db = Path(temp) / "avatar.sqlite3"
