"""Cached Avatar MP4 player for the centre overlay (Phase 4)."""
...
class AvatarPlayer:
    ...
    def play(self, path: str | Path, size: tuple[int, int]):
        ...
        if not ffmpeg: raise RuntimeError("ffmpeg is required for Avatar playback")
        ...
        threading.Thread(target=self._decode_frames, args=(self.process, self.width, self.height, self._frames), daemon=True, name="avatar-video-decode").start()
    ...
    def update(self, pygame_module):
        if not self.process:
            if self.surface is not None and self.ends_at and time.monotonic() >= self.ends_at:
                self.surface = None
                self.ends_at = 0.0
            return
    ...
    def stop(self):
        ...
        self.process = self.audio = self.audio_decoder = None; self.surface = None; self._frames = Queue(); self.frames_decoded = 0; self.started_at = 0.0; self.ends_at = 0.0
    ...
    def _finish(self):
        ...
        self.ends_at = time.monotonic() + self.linger_seconds
