...
FINAL_REFERENCE_HOLD_SECONDS = 1.0
BASE_PROMPT = (
...
def _hold_reference_final(video: Path, destination: Path) -> None:
    """Make the final second pixel-derived from the approved reference image."""
    ffmpeg = shutil.which("ffmpeg")
    if not ffmpeg:
        raise PrincessConfigurationError("ffmpeg is required to hold the approved reference at the end")
    partial = destination.with_suffix(".partial.mp4")
    filter_graph = (
        "[0:v]trim=duration=4,setpts=PTS-STARTPTS,scale=480:704:force_original_aspect_ratio=decrease,"
        "pad=480:704:(ow-iw)/2:(oh-ih)/2:color=black[v0];"
        "[1:v]trim=duration=1,setpts=PTS-STARTPTS,scale=480:704:force_original_aspect_ratio=decrease,"
        "pad=480:704:(ow-iw)/2:(oh-ih)/2:color=black[v1];[v0][v1]concat=n=2:v=1:a=0[v]"
    )
    command = [
        ffmpeg, "-y", "-i", str(video), "-loop", "1", "-i", str(REFERENCE), "-filter_complex", filter_graph,
        "-map", "[v]", "-map", "0:a:0?", "-shortest", "-c:v", "libx264", "-preset", "fast", "-crf", "20",
        "-pix_fmt", "yuv420p", "-c:a", "copy", "-movflags", "+faststart", str(partial),
    ]
    completed = subprocess.run(command, capture_output=True, text=True, timeout=300, check=False)
    if completed.returncode != 0 or not partial.is_file() or partial.stat().st_size == 0:
        partial.unlink(missing_ok=True)
        raise PrincessServiceError(f"Failed to hold reference as final frame: {completed.stderr.strip()[-500:]}")
    os.replace(partial, destination)
...
def generate(name: str, prompt: str, *, model: str, version: str, overwrite: bool, sound_effect: Path | None) -> dict:
    ...
    transition_output = output.with_suffix(".transition.mp4")
    ...
    try:
        ...
        _hold_reference_final(provider_output, transition_output)
        _mux_sound_effect(transition_output, effect, output)
    finally:
    ...
    payload = {
        ...
        "prompt": prompt,
        "final_reference_hold_seconds": FINAL_REFERENCE_HOLD_SECONDS,
...
def finalize_existing_catalogue() -> list[str]:
    """Apply the exact reference hold locally to clips generated by older tool versions."""
    finalized = []
    for clip in sorted(OUTPUT_DIR.glob("*.mp4")):
        temporary = clip.with_suffix(".refhold.mp4")
        _hold_reference_final(clip, temporary)
        os.replace(temporary, clip)
        metadata = clip.with_suffix(".json")
        if metadata.is_file():
            payload = json.loads(metadata.read_text(encoding="utf-8"))
            payload["final_reference_hold_seconds"] = FINAL_REFERENCE_HOLD_SECONDS
            payload["media"] = inspect_media(clip)
            atomic_write_json(metadata, payload)
        finalized.append(str(clip))
    return finalized
...
def build_parser() -> argparse.ArgumentParser:
    ...
    action.add_argument("--finalize-existing", action="store_true", help="locally enforce the exact reference final second for existing clips")
...
def main() -> int:
    ...
    if args.finalize_existing:
        try:
            print(json.dumps({"finalized": finalize_existing_catalogue()}, indent=2))
            return 0
        except (PrincessConfigurationError, PrincessServiceError) as exc:
            print(f"Apparition finalisation failed safely: {exc}", file=sys.stderr)
            return 1
    if not report["ok"]:
