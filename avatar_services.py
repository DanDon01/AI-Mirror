"""Provider and media helpers for the standalone Avatar video proof.

This module deliberately has no Pygame dependency. Provider SDK imports are
lazy so the normal mirror and offline tests still start when Avatar is off.
"""
...
class AvatarConfigurationError(AvatarServiceError):
...
def _existing_file(path: str | Path, label: str) -> Path:
    ...
    if not candidate.is_file():
        raise AvatarConfigurationError(f"{label} does not exist: {candidate}")
    return candidate
...
class OpenAITTSService:
    ...
    def synthesize(
        ...
        *,
        ...
        if not spoken:
            raise AvatarConfigurationError("TTS text must not be empty")
        api_key = os.getenv("OPENAI_API_KEY", "").strip()
        if not api_key:
            raise AvatarConfigurationError("OPENAI_API_KEY is not configured")
        if self.tracker and not self.tracker.allow(AVATAR_MODULE, OPENAI_TTS_SERVICE):
            raise AvatarServiceError("OpenAI request blocked by API usage policy")
        ...
        try:
            ...
            if not partial.is_file() or partial.stat().st_size == 0:
                raise AvatarServiceError("OpenAI returned an empty speech file")
            os.replace(partial, output)
            elapsed = time.monotonic() - started
            if self.tracker:
                # The speech response does not expose billed audio-token usage.
                self.tracker.record(AVATAR_MODULE, OPENAI_TTS_SERVICE, estimated_cost=0.0)
            return TTSResult(
                path=str(output),
            ...
            if self.tracker:
                self.tracker.failure(AVATAR_MODULE, OPENAI_TTS_SERVICE)
            if isinstance(exc, AvatarServiceError):
                raise
            raise AvatarServiceError(f"OpenAI TTS failed: {exc}") from exc
...
class FlashTalkService:
    ...
    def generate(
        ...
        *,
        ...
        if not fal_key:
            raise AvatarConfigurationError("FAL_KEY is not configured")
        if not os.getenv("FAL_KEY", "").strip():
            os.environ["FAL_KEY"] = fal_key
        if self.tracker and not self.tracker.allow(AVATAR_MODULE, FAL_AVATAR_SERVICE):
            raise AvatarServiceError("fal request blocked by API usage policy")
        ...
        try:
            ...
            if not video_url:
                raise AvatarServiceError("fal response did not include video.url")
            ...
            if not partial.is_file() or partial.stat().st_size == 0:
                raise AvatarServiceError("fal returned an empty video file")
            os.replace(partial, output)
            ...
            partial.unlink(missing_ok=True)
            if self.tracker:
                self.tracker.failure(AVATAR_MODULE, FAL_AVATAR_SERVICE)
            if isinstance(exc, AvatarServiceError):
                raise
            suffix = f" (request {request_id})" if request_id else ""
            raise AvatarServiceError(f"fal generation failed{suffix}: {exc}") from exc
    ...
    def generate_from_text(
        ...
        *,
        ...
        if not spoken and not allow_silent:
            raise AvatarConfigurationError("Avatar text must not be empty")
        if not spoken and "ai-avatar/single-text" in model:
            raise AvatarConfigurationError("The single-text avatar endpoint requires spoken text")
        if not os.getenv("FAL_KEY", "").strip() and os.getenv("FAL", "").strip():
            os.environ["FAL_KEY"] = os.getenv("FAL", "").strip()
        if not os.getenv("FAL_KEY", "").strip():
            raise AvatarConfigurationError("FAL_KEY is not configured")
        if not self.tracker or self.tracker.allow(AVATAR_MODULE, FAL_AVATAR_SERVICE):
            pass
        else:
            raise AvatarServiceError("fal request blocked by API usage policy")
        ...
        try:
            ...
            if self.tracker:
                self.tracker.record(AVATAR_MODULE, FAL_AVATAR_SERVICE, estimated_cost=cost or 0.0)
            progress_at = first_progress_at or generation_done
            provider_timings = result.get("timings", {}) if isinstance(result, dict) else {}
            if not isinstance(provider_timings, dict):
                provider_timings = {}
            ...
            if isinstance(exc, AvatarServiceError):
                raise
            suffix = f" (request {request_id})" if request_id else ""
            raise AvatarServiceError(f"fal text-avatar generation failed{suffix}: {exc}") from exc
...
def inspect_media(path: str | Path, ffprobe: str | None = None, *, require_audio: bool = True) -> dict[str, Any]:
    ...
    if not executable:
        raise AvatarConfigurationError("ffprobe is not installed or on PATH")
    command = [
        executable,
        "-v",
        "error",
    ...
    if completed.returncode != 0:
        ...
        raise AvatarServiceError(f"ffprobe rejected the generated video: {detail}")
    try:
        raw = json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise AvatarServiceError("ffprobe returned malformed JSON") from exc

    streams = raw.get("streams", [])
    video = next((item for item in streams if item.get("codec_type") == "video"), None)
    audio = next((item for item in streams if item.get("codec_type") == "audio"), None)
    if not video:
        raise AvatarServiceError("Generated MP4 has no video stream")
    if not audio and require_audio:
        raise AvatarServiceError("Generated MP4 has no audio stream")
...
def normalize_video(
    ...
    if not executable:
        raise AvatarConfigurationError("ffmpeg is not installed or on PATH")
    output = Path(destination).resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    ...
    if completed.returncode != 0:
        ...
        raise AvatarServiceError(f"ffmpeg normalization failed: {detail}")
    if not partial.is_file() or partial.stat().st_size == 0:
        raise AvatarServiceError("ffmpeg produced an empty playback file")
    os.replace(partial, output)
